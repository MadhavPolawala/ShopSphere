        ...
        <div className="flex items-center gap-1 mb-2 sm:mb-4">
          <div className="flex">
            {[...Array(5)].map((_, i) => {
              ...
              return (
                <div key={i} className="relative w-2.5 h-2.5 sm:w-4 sm:h-4">
                  {/* Background Empty Star */}
                  <svg className="w-full h-full fill-current text-gray-200 dark:text-gray-700" viewBox="0 0 20 20">
                    <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
                  </svg>
                  {/* Foreground Filled Star */}
                  <svg
                    className="absolute top-0 left-0 w-full h-full fill-current text-yellow-400"
          ...
          <span className="text-[9px] sm:text-xs text-gray-400 font-bold ml-1">
            {product.rating ? product.rating.toFixed(1) : '0.0'} <span className="opacity-70 ml-1">({product.numReviews || 0})</span>
          </span>
        </div>
